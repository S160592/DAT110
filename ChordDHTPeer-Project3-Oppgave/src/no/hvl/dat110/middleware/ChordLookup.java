...
public class ChordLookup {
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		BigInteger nodeID = this.node.getNodeID();
		List<NodeInterface> fingers = this.node.getFingerTable();
		...
		for (int i = 0; i < fingers.size() -1; i++) {
			...
			NodeInterface ftsucc = fingers.get(m);
			
			try {
				BigInteger ftsuccID = ftsucc.getNodeID();
				
				NodeInterface ftsuccnode = Util.getProcessStub(ftsucc.getNodeName(), ftsucc.getPort());
				
				boolean cond = Util.computeLogic(ftsuccID, this.node.getNodeID().add(new BigInteger("1")), key.subtract(new BigInteger("1")));
