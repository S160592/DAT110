...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		Boolean permitted = areAllMessagesReturned(activeNodes.size());
		// if yes, acquireLock
		if (permitted) {
			acquireLock();
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		for (Message msg : activenodes) {
			Util.getProcessStub(msg.getNodeIP(), msg.getPort()).onMutexRequestReceived(message);
		}
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		for (Message msg : activenodes) {
			try {
				Util.getProcessStub(msg.getNodeIP(), msg.getPort()).releaseLocks();
